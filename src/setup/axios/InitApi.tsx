...
export const getDoanhthuPlatformMonthly = async (params: any) => {
  try {
    const response = await API.get(
      `/platform?skip=${params.skip}&limit=${params.limit}`
    );
    if (response) {
      return response.data;
    }
...
export const getFileDoanhthuPlatform = async (params: any) => {
  try {
    const response = await APIExport.get(
      `/platform/export-excel?skip=${params.skip}&limit=${params.limit}&isCurrentPage=${params.isCurrentPage}&month=${params.month}`
    );
    if (response) {
      return response;
    }
